...
private struct WorkspaceFileDTO: Codable {
    ...
    struct CodexDTO: Codable {
        var auth: CodexAuthDTO?
    }

    struct CodexAuthDTO: Codable {
        var mode: String
        var secret: String
        var version: String?
        var url: String?
    ...
    private static func serviceDTO(from s: WorkspaceRepoServiceConfig) -> ServiceDTO {
        ServiceDTO(
            ...
            codex: s.codex.map { CodexDTO(auth: $0.auth.map { CodexAuthDTO(mode: $0.mode, secret: $0.secret, version: $0.version, url: $0.url) }) },
    ...
    private static func serviceConfigFromDTO(_ s: ServiceDTO) -> WorkspaceRepoServiceConfig {
        WorkspaceRepoServiceConfig(
            ...
            codex: s.codex.map { WorkspaceRepoCodexConfig(auth: $0.auth.map { WorkspaceRepoCodexAuthConfig(mode: $0.mode, secret: $0.secret, version: $0.version, url: $0.url) }) },
