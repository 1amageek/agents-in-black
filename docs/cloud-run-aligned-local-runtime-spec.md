...
- `workspace.yaml`: ローカル runtime の唯一の設定ソース（source of truth）
...
`workspace.yaml` を runtime 設定の唯一の source of truth とする。
...
`swift-configuration` は複数 provider の合成を提供するが、本仕様における source of truth は引き続き `workspace.yaml` とする。

v1 では次のルールを適用する。

- `workspace.yaml` は再現性のある構造定義（サービス一覧・ルーティング・起動コマンド）の正本
...
- 単一ポート公開（例: `:8080`）
- workspace.yaml 由来のルーティング
- HTTP 契約の透過転送 + 最小限の補正

## 6.2 パスベースルーティング

ルーティングテーブルは `workspace.yaml` から生成する。
...
## 7.6 構造変更対応（workspace.yaml 差分適用）
...
- `workspace.yaml` が唯一の source of truth
...
- `workspace.yaml` の source-of-truth 性を崩す override 運用
...
- `workspace.yaml` JSON Schema もしくは厳密バリデータ仕様
